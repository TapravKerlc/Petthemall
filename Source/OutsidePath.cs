...
namespace PetThemAll{
    public class OutsidePath_JoyGiver : JoyGiver {
        public override Job TryGiveJob(Pawn pawn) {
            ...
            var pettingAnimal = Common.GetAnimal(pawn);
            if (pettingAnimal is null) {
                PetThemAll.Debug($"no valid animal found");
                return null;
            }

            if (!Common.TryFindOutsideWalkPath(pawn, pettingAnimal, out var firstCell, out var queue)) {
                PetThemAll.Debug($"no path");
                return null;
            }

            var job = JobMaker.MakeJob(this.def.jobDef, firstCell.Value, pettingAnimal);
            job.targetQueueA = queue;
            job.locomotionUrgency = LocomotionUrgency.Jog;
            PetThemAll.Debug($"found animal {pettingAnimal.ToStringSafe()}, made job {job.ToStringSafe()}");
            return job;
        }
    }
    ...
    public abstract class OutsidePath_JobDriver : JobDriver {
        ...
        protected Pawn pettingAnimal => (Pawn) this.WalkingAnimalTargetInfo.Thing;
        ...
        protected bool AnimalIsFailed() => Common.PawnOrAnimalIsGoneOrIncapable(this.pettingAnimal);
        ...
        protected bool AnimalHasWrongJob() {
            ...
            var curJob = this.pettingAnimal.jobs.curJob;
        ...
        protected void SetAnimalJob(Job animalJob) {
            ...
            var animalJobs = this.pettingAnimal.jobs;
            animalJobs.StopAll();
        ...
        protected Action MakeChatWithAnimal() => delegate {
            this.pawn.interactions.TryInteractWith(this.pettingAnimal, InteractionDefOf.AnimalChat);
        };
        ...
        protected void UnqueueExpectedJob() {
            ...
            if (expectedAnimalJob != null) {
                var pettingAnimal = this.pettingAnimal;
                if (!Common.PawnOrAnimalIsGone(pettingAnimal)) {
                    pettingAnimal.jobs.EndCurrentOrQueuedJob(expectedAnimalJob, JobCondition.Succeeded);
                }
            }
        }
    }
